...
class FallbackTeamSkillSelectorV4:
    ...
    def __init__(self, client: CompletionClient, prompt_path: str | Path | None = None) -> None:
        ...
        self.prompt_path = Path(
            prompt_path or root / "config" / "team_skill_fallback_selector_v4.txt"
        )
    ...
    def _complete_json(self, system_prompt: str, user_prompt: str) -> LLMCompletion:
        """Call the retrieval selector with enough output budget for reasoning models.

        Provider/protocol failures are normalized into ``FallbackSelectorError`` so
        the existing pipeline reliability guard can preserve recall instead of
        crashing the entire resume run.
        """
        try:
            ...
            raise FallbackSelectorError(
                f"fallback selector model call failed: {type(exc).__name__}"
            ) from None
