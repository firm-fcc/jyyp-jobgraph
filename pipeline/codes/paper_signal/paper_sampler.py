# -*- coding: utf-8 -*-
"""通用分层抽样引擎：对 (stratum, text) 条目按层抽样（自持拷贝，与 builder/sampler.py 同源）。

设计：**数据源无关**。不关心文本来自 JD / 论文 / 新闻，只负责：
- 按层分配抽样量（min_coverage / proportional / uniform），保证总目标不超过 n；
- sample() 覆盖全部层（冷启动）；next_batch() 按层占比且跨批不重复（热更新）；
- 加载时按文本去重、断点继续（保存已消费文档 md5）。

论文场景：stratum=分档（或 分档|主维度），text=论文紧凑上下文（标题+摘要+关键词+证据句+正文片段）。
实际 LLM 输入使用 PaperRecord 全字段，sampler 的 text 仅用于去重与断点。
"""
import hashlib
import json
import os
